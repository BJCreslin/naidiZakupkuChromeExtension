...
async function isUserAuthorized() {
    return new Promise((resolve) => {
        chrome.runtime.sendMessage({ destination: MESSAGE_TYPES.CHECK_AUTH }, (response) => {
...
const cancelledStatusConfig = {
     ...
     * @returns true если статус в списке исключений
     */
    isStatusExcluded(status) {
        return this.excludedStatuses.some(excludedStatus => status && status.includes(excludedStatus));
    },
    /**
...
window.procurementStatusManager = {
     ...
     * @returns JSON строка с конфигурацией
     */
    exportConfig: () => {
        const config = {
     ...
     * @param jsonConfig - JSON строка с конфигурацией
     */
 ...
 * @param procurementNumber - Номер закупки
 */
 ...
 * @param timeZoneString - Строка с временной зоной
 * @returns Только UTC часть без скобок или исходная строка
 */
 ...
 * @param input - Исходная строка с ценой
 * @returns Нормализованная цена
 */
 ...
 * @returns true если закупка отменена
 */
...
// Абстрактный класс для парсера закупок
...
// Парсер для 223-ФЗ
class ProcurementParser223 extends ProcurementParser {
    getFederalLawNumber() {
        return "223";
    }
    getRegistryNumber() {
...
function addCss(css) {
    ...
    if (head.querySelector(`link[href="${css}"]`))
        return;
...
async function insertButton(className) {
    ...
    if (!isAuthorized) {
        ...
        showNotification("🔐 Требуется авторизация", "Для сохранения закупок откройте расширение и войдите в систему", 'info');
        return;
    }
    ...
    if (!buttonPlace.querySelector("input[type=button]")) {
        ...
        buttonToMHelper.onclick = function () {
            chrome.runtime.sendMessage({
                destination: MESSAGE_TYPES.PROCUREMENT_SENDER,
...
function fillProcurementWith615And44() {
    ...
    if (lawNumberElement) {
        dataAboutProcurement.federalLawNumber = lawNumberElement.textContent?.split("\n")[0] || "";
    }
    else {
    ...
    if (registryNumberElement) {
        const registryNumberText = registryNumberElement.textContent?.trim() || "";
    ...
    if (nameElement) {
        dataAboutProcurement.name = nameElement.textContent?.trim() || "";
    }
    else {
    ...
    if (publisherElement) {
        dataAboutProcurement.publisher = publisherElement.textContent?.trim() || "";
    }
    else {
    ...
    if (timeZoneElement) {
        const fullTimeZone = timeZoneElement.textContent?.trim() || "";
